...
def get_file_path(instance, filename):
	...
	filename = "src.%s" % (ext)
	return os.path.join('submitions/', '%s/' % instance.contest.short_name, 'user_%s/' % instance.user.id, 'submition_%s/' % time.strftime("%Y%m%d%H%M%S"), filename)
...
class Atempt(models.Model):
	...
	team    = models.ForeignKey(Team, default=1, null=True, on_delete=models.SET_NULL) # this is redundant but it's just easier...
	date = models.DateTimeField(auto_now_add=True, blank=True)
	file   = models.FileField(upload_to=get_file_path, blank=False, null=False, max_length=512, validators=[validate_file_extension])
