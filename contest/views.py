...
from .forms import AttemptModelForm, TeamModelForm, TeamMemberForm, TeamMemberApprovalForm, \
    ProfileEditForm, UserEditForm, GroupJoinForm
from .models import Contest, UserContestDateException, Group
...
def contest_list_view(request):
    ...
    if request.user.is_superuser:
        ...
        contests_qs = Contest.getContestsForUser(request.user)
...
def group_list_view(request):
    template_name = 'contest/group_list.html'

    context = {}
    groups = Group.getGroupsForUser(request.user)
    context.update({
        'groups': groups
    })
    return render(request, template_name, context)
...
def group_join_view(request):
    template_name = 'contest/group_join.html'
