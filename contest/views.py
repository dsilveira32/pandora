...
@login_required
def atempt_create_view(request, id):
...
def team_create_view(request, id):
	...
	user_team = TeamMember.objects.select_related('team').filter(team__contest = contest_obj.id, user = request.user).first()
...
def team_detail_view(request, id):
	...
	can_be_deleted = True
	if members.count() > 1 or not team_member_obj.approved or get_team_atempts(team_obj):
...
@login_required
def atempt_list_view(request, id):
